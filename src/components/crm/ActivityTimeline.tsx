...
export function ActivityTimeline({ candidateId, activities: initialActivities }: ActivityTimelineProps) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (!res.error && res.id) {
      ...
      setForm({ type: 'Reunião', date: '', status: 'Pendente', note: '', external_link: '' })
      setShowForm(false)
    }
    setLoading(false)
  }
  ...
  async function handleStatusChange(id: string, status: string) {
    await updateActivity(id, { status: status as 'Pendente' | 'Agendada' | 'Concluída' | 'Reagendada' | 'Cancelada' })
    setActivities((prev) =>
            ...
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {overdue && (
                    <Badge variant="red" className="text-[10px]">Atrasada</Badge>
                  )}
                ...
                {act.note && (
                  <p className="text-xs text-text-secondary mt-1">{act.note}</p>
                )}
                {act.external_link && (
                  <a
